...
import com.serviciosweb.intCoperah.db.ConnectionOki;
import com.serviciosweb.intCoperah.db.ConnectionSana;
...
public class ProcessSana extends Thread{
	...
	public void ejecutaProceso(){
		...
		//fechaFin = "03/03/2014";
		type = "D";
		ConnectionSana CF = new ConnectionSana();
		
		ConnectionFybeca CFy = new ConnectionFybeca();
		try {
			CallableStatement cst = CF.ConectToSana().prepareCall("{call integraciones.int_pkg_integrac.INT_SP_CREATECURTRANSCO(?,?,?,?,?)}");
	...
	public void procesoCarga(InfoCarga carga){
		...
		ConnectionSana nConn = new ConnectionSana();
		
		nConn.saveResponse("TRANSACCIONAL SANA", estado, detallesR , codCarga);
		
		if(response.getEstado().equals("ERROR")){
			System.out.println("ERROR SANA");
			...
			if(detalles.size()>0){
				for (String det : detalles) {
					System.out.println("DETALLE SANA: "+det);
				}
				
			}
		}else{
			System.out.println("RESPUESTA: SANA"+response.getEstado());
			ArrayOfString detalle = new ArrayOfString();
			detalle = response.getDetalle();
			List<String> detalles = new ArrayList<String>();
			detalles = detalle.getString();
			...
			System.out.println("ID CARGA SANA: "+response.getIdCarga());
		}
	}
}
