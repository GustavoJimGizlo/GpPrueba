...
public class ProcessOki extends Thread{
	...
	public void ejecutaProceso(){
		...
		//fechaFin = "03/03/2014";
		type = "D";
		ConnectionOki CF = new ConnectionOki();
		
		ConnectionFybeca CFy = new ConnectionFybeca();

		try {
			CallableStatement cst = CF.ConectToOki().prepareCall("{call integraciones.int_pkg_integrac.INT_SP_CREATECURTRANSCO(?,?,?,?,?)}");
	...
	public void procesoCarga(InfoCarga carga){
		...
		ConnectionOki nConn = new ConnectionOki();
		
		nConn.saveResponse("TRANSACCIONAL OKI", estado, detallesR , codCarga);
		
		if(response.getEstado().equals("ERROR")){
			System.out.println("ERROR OKI");
			...
			if(detalles.size()>0){
				for (String det : detalles) {
					System.out.println("DETALLE OKI: "+det);
				}
				
			}
		}else{
			System.out.println("RESPUESTA OKI: "+response.getEstado());
			ArrayOfString detalle = new ArrayOfString();
			detalle = response.getDetalle();
			List<String> detalles = new ArrayList<String>();
			detalles = detalle.getString();
			...
			System.out.println("ID CARGA OKI: "+response.getIdCarga());
		}
	}
}
