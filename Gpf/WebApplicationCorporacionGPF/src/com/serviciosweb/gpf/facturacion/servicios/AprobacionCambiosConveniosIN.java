...
public class AprobacionCambiosConveniosIN extends ObtenerNuevaConexion {
	...
	public AprobacionCambiosConveniosIN() {
		super(AprobacionCambiosConveniosIN.class);
	}

	public AprobacionCambiosConveniosIN(String convenios, String url, String usuarios, String empresas, String cupos,String nombres_empresas, String status) {
		super(AprobacionCambiosConveniosIN.class);
		...
		try {
			...
			if (conn.getConn() == null) {
				...
				respuestaXml.append(
						"<html><body><p>&nbsp;</p><p style=\"color:Red; text-align:center; font-size:200%\"><strong>ERROR</strong></p><p>&nbsp;</p>\r\n"
				...
				sSQL = "SELECT CAST ((sysdate - fecha_solicitud) as numeric(36,2)) as tiempo from ingresos.in_autorizar_cupo where estado_solicitud = 'S' and convenio ='"
				...
				if (conn.siguiente(rs)) {
					...
					if ((caracter == 'A') && (Double.parseDouble(rs_tiempo) <= periodo)) {
						...
						sSQL = "UPDATE ingresos.in_clientes_ingresos_param  SET campo1 = '" + cupo + "'  WHERE codigo = '"
								+ convenio + "'";
						conn.getConn().commit();
						rs = conn.consulta(sSQL);
						if (conn.siguiente(rs)) {
							sSQL = "UPDATE ingresos.in_autorizar_cupo SET estado_solicitud='" + status
						...
						sSQL = "UPDATE ingresos.in_autorizar_cupo SET estado_solicitud='R', estado_aprobacion='"+ status_final +"', fecha_aprobacion=sysdate, usuario_aprobacion='"
